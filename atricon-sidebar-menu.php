...
add_action('wp_head', function(){
    ...
    ?>    <style>    /* === SIDEBAR PRINCIPAL - DESIGN MODERNO E RESPONSIVO === */
    #atricon-sidebar{
        position:fixed;
        top:0;
        <?php echo $side_css; ?>
    ...
    #atricon-sidebar::-webkit-scrollbar-track {
    ...
    #atricon-sidebar:hover{
        width:<?php echo $expanded_width; ?>;
        box-shadow: <?php echo $side === 'right' ? '-6px 0 30px rgba(0,0,0,0.15)' : '6px 0 30px rgba(0,0,0,0.15)'; ?>;
    ...
    @media (max-width: 1024px) {
        #atricon-sidebar {
            width: <?php echo $behavior === 'always_show' ? '240px' : '50px'; ?>;
        }
    ...
    /* Responsividade para mobile - sidebar oculto por padrão */
    @media (max-width: 768px) {
        ...
        #atricon-sidebar.mobile-active {
            transform: translateX(0);
        }
        
        /* Botão para ativar menu mobile */
    ...
    }      /* === LISTA DE MENU - MELHOR ORGANIZAÇÃO === */
    .atricon-menu{
        list-style:none;
        padding:0;
        margin:0;
    ...
    .atricon-item:has(.atricon-submenu) > .atricon-link::after {
        content: '';
        position: absolute;
        right: 16px;
        top: 50%;
    ...
    <?php if ($behavior === 'icon_only'): ?>
    #atricon-sidebar:hover .atricon-item:has(.atricon-submenu) > .atricon-link::after {
        opacity: 1;
    }
    <?php else: ?>
    .atricon-item:has(.atricon-submenu) > .atricon-link::after {
    ...
    <?php if ($behavior === 'icon_only'): ?>
    #atricon-sidebar:hover ._mi._before {
        margin-right:12px;
    }
    <?php else: ?>
    ._mi._before {
    ...
    <?php if ($behavior === 'icon_only'): ?>
    #atricon-sidebar:hover .atricon-icon {
        margin-right:12px;
    }
    <?php else: ?>
    .atricon-icon {
    ...
    /* === CONTROLE DE VISIBILIDADE DOS TEXTOS === */
    <?php if ($behavior === 'icon_only'): ?>
    /* No modo icon_only, esconde textos inicialmente */
    ...
    <?php else: ?>
    /* No modo always_show, sempre mostra os textos */
    #atricon-sidebar .atricon-link .menu-icon-label,
    #atricon-sidebar .atricon-link span:not([class*="dashicons"]):not([class*="_mi"]),
    #atricon-sidebar .atricon-label {
        opacity:1;
    ...
    /* Estilo específico para texto dos links */
    .atricon-link > span:not([class*="dashicons"]):not([class*="_mi"]) {
        font-weight:500;
        line-height: 1.4;
    }    
    /* === SUBMENUS MELHORADOS - SEM CORTES === */
    .atricon-submenu{
        display:none;
        list-style:none;
        padding:0;
        ...
        max-height: 400px; /* Limita altura para evitar cortes */
        overflow-y: auto; /* Scroll interno se necessário */
        position: relative;
        z-index: 10;
    }
    
    /* Scrollbar para submenus */
    .atricon-submenu::-webkit-scrollbar {
        width: 3px;
    }
    
    .atricon-submenu::-webkit-scrollbar-thumb {
        background-color: rgba(0,0,0,0.2);
        border-radius: 2px;
    }
    
    /* Controla quando mostrar submenu */
    <?php if ($behavior === 'icon_only'): ?>
    /* Só mostra submenu quando sidebar expandido E em hover */
    #atricon-sidebar:hover .atricon-item:hover > .atricon-submenu{
        display:block;
        animation: slideDown 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    }
    <?php else: ?>
    /* No modo always_show, mostra submenu no hover */
    ...
    <?php if ($behavior === 'icon_only'): ?>
    #atricon-sidebar:hover .atricon-footer{
        opacity:1;
        width: <?php echo $expanded_width; ?>;
    }
    <?php else: ?>
    ...
    .atricon-footer:hover .atricon-logo-img {
        transform: scale(1.1);
    }
    ...
    #atricon-sidebar *,
    #atricon-sidebar *::before,
    #atricon-sidebar *::after {
        transition:all .3s cubic-bezier(0.4, 0, 0.2, 1);
    }
    
    /* Melhora a visibilidade no estado contraído */
    <?php if ($behavior === 'icon_only'): ?>
    #atricon-sidebar:not(:hover) .atricon-link {
        justify-content:center;
        padding: 16px 12px;
    }
    <?php endif; ?>
    ...
    /* Prevenção de corte de conteúdo */
    #atricon-sidebar {
    ...
    /* Estado loading/carregamento */
    .atricon-loading {
        pointer-events: none;
        opacity: 0.6;
    }
    
    .atricon-loading::after {
        content: '';
        position: absolute;
        top: 50%;
        left: 50%;
    ...
    .atricon-search-wrapper:focus-within .atricon-search-icon {
        color: #2271b1;
    }
    ...
    .atricon-search-input::placeholder {
        color: #9ca3af;
        font-style: normal;
        font-weight: 400;
    }    
    <?php if ($behavior === 'icon_only'): ?>
    /* === MODO ICON-ONLY RESPONSIVO === */
    #atricon-sidebar:not(:hover) .atricon-search-container {
        padding: 10px;
        background: rgba(248, 250, 252, 0.8);
        border-bottom: 1px solid rgba(0,0,0,0.05);
    }
    
    #atricon-sidebar:not(:hover) .atricon-search-wrapper {
        width: 40px;
        height: 40px;
        border-radius: 50%;
        justify-content: center;
    ...
    #atricon-sidebar:not(:hover) .atricon-search-icon {
        padding: 0;
        margin: 0;
        position: absolute;
        top: 50%;
    ...
    #atricon-sidebar:not(:hover) .atricon-search-input {
        position: absolute;
        left: -9999px;
        opacity: 0;
        width: 1px;
    ...
    /* Hover - Expansão suave */
    #atricon-sidebar:hover .atricon-search-container {
        padding: 16px 12px;
        background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%);
    }
    
    #atricon-sidebar:hover .atricon-search-wrapper {
        width: 100%;
        height: 40px;
        border-radius: 8px;
    ...
    #atricon-sidebar:hover .atricon-search-icon {
        position: static;
        transform: none;
        padding: 10px;
        color: #6b7280;
    ...
    #atricon-sidebar:hover .atricon-search-input {
        position: static;
        left: auto;
        opacity: 1;
        width: 100%;
    ...
    /* === RESULTADOS DE BUSCA OTIMIZADOS - SEM CORTES === */
    ...
    .atricon-search-results::-webkit-scrollbar-track {
        background: #f8f9fa;
    }
    ...
    .atricon-search-result-item::before {
        content: '';
        position: absolute;
        left: 0;
        top: 0;
    ...
    .atricon-search-result-item:hover::before,
    .atricon-search-result-item.atricon-active::before {
        width: 3px;
    }
    
    .atricon-search-result-item:last-child {
        border-bottom: none;
        border-radius: 0 0 8px 8px;
    }
    ...
    /* Ajustes especiais para modo icon_only */
    <?php if ($behavior === 'icon_only'): ?>
    #atricon-sidebar:not(:hover) .atricon-search-results {
        display: none !important;
    }
    
    #atricon-sidebar:hover .atricon-search-results {
        margin-top: 0;
        border-radius: 0 0 8px 8px;
    }
    <?php endif; ?>
...
add_action('wp_footer', function(){
    if (atricon_menu_icons_active() && function_exists('menu_icons')) {
        ...
        jQuery(document).ready(function($) {
            ...
            // === MELHORIAS NO COMPORTAMENTO DOS SUBMENUS ===
            let submenuTimeout;
            
            $('#atricon-sidebar .atricon-item').on('mouseenter', function() {
                clearTimeout(submenuTimeout);
                const $item = $(this);
                const $submenu = $item.find('.atricon-submenu');
                
                if ($submenu.length && ($('#atricon-sidebar').is(':hover') || $('#atricon-sidebar').width() > 100)) {
                    // Calcula posição para evitar corte na tela
                    const itemOffset = $item.offset();
                    const submenuHeight = $submenu.outerHeight();
                    const windowHeight = $(window).height();
                    const spaceBelow = windowHeight - itemOffset.top;
                    
                    if (submenuHeight > spaceBelow && itemOffset.top > submenuHeight) {
                        $submenu.css({
                            'transform': 'translateY(-' + (submenuHeight - $item.outerHeight()) + 'px)',
                            'max-height': Math.min(400, windowHeight - 100) + 'px'
                        });
                    } else {
                        $submenu.css({
                            'transform': 'translateY(0)',
                            'max-height': Math.min(400, spaceBelow - 50) + 'px'
                        });
                    }
                ...
                const $submenu = $(this).find('.atricon-submenu');
            ...
            // Garantir que o sidebar seja scrollável quando necessário
            $('#atricon-sidebar').on('mouseenter', function() {
                $(this).addClass('expanded');
                // Permite scroll quando expandido
                ...
                // Controla overflow baseado no comportamento
                <?php if ($behavior === 'icon_only'): ?>
        ...
        jQuery(document).ready(function($) {
            ...
            $('#atricon-sidebar .atricon-item').on('mouseenter', function() {
                if ($('#atricon-sidebar').is(':hover')) {
                    $(this).find('.atricon-submenu').stop(true, true).slideDown(200);
                }
            }).on('mouseleave', function() {
                $(this).find('.atricon-submenu').stop(true, true).slideUp(150);
            });
